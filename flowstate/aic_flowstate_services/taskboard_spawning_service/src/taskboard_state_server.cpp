...
class TaskboardStateServer : public rclcpp::Node {
 public:
  TaskboardStateServer() : Node("taskboard_state_server") {
    ...
    if (!config_file.empty()) {
      ...
      RCLCPP_WARN(this->get_logger(), "No config file provided, starting with empty state.");
    }

    update_service_ = this->create_service<aic_flowstate_interface::srv::UpdateComponent>(
        "update_component",
        std::bind(&TaskboardStateServer::HandleUpdateComponent, this, std::placeholders::_1, std::placeholders::_2));

    get_state_service_ = this->create_service<aic_flowstate_interface::srv::GetTaskboardState>(
        "get_taskboard_state",
        std::bind(&TaskboardStateServer::HandleGetTaskboardState, this, std::placeholders::_1, std::placeholders::_2));
 ...
 private:
  void LoadDefaultState(const std::string& file_path) {
    try {
      ...
      if (config["trials"]) {
        ...
        for (YAML::const_iterator it = config["trials"].begin(); it != config["trials"].end(); ++it) {
          if (it->second["scene"] && it->second["scene"]["task_board"]) {
            state_node_ = it->second["scene"]["task_board"];
            RCLCPP_INFO(this->get_logger(), "Loaded default state from %s", file_path.c_str());
            return;
          }
        }
      }
      RCLCPP_WARN(this->get_logger(), "No valid scene found in %s", file_path.c_str());
  ...
  void HandleUpdateComponent(
    ...
    RCLCPP_INFO(this->get_logger(), "Received update for %s", request->component_name.c_str());
  ...
  void HandleGetTaskboardState(
      const std::shared_ptr<aic_flowstate_interface::srv::GetTaskboardState::Request> /*request*/,
      std::shared_ptr<aic_flowstate_interface::srv::GetTaskboardState::Response> response) {
    
    RCLCPP_INFO(this->get_logger(), "State requested and served.");

    // Fill task_board base pose
    if (state_node_["pose"]) {
      response->task_board_x = state_node_["pose"]["x"] ? state_node_["pose"]["x"].as<double>() : 0.0;
      response->task_board_y = state_node_["pose"]["y"] ? state_node_["pose"]["y"].as<double>() : 0.0;
      response->task_board_z = state_node_["pose"]["z"] ? state_node_["pose"]["z"].as<double>() : 0.0;
      response->task_board_roll = state_node_["pose"]["roll"] ? state_node_["pose"]["roll"].as<double>() : 0.0;
      response->task_board_pitch = state_node_["pose"]["pitch"] ? state_node_["pose"]["pitch"].as<double>() : 0.0;
      response->task_board_yaw = state_node_["pose"]["yaw"] ? state_node_["pose"]["yaw"].as<double>() : 0.0;
    }
    ...
    auto fill_comp = [&](const std::string& key, auto& field) {
      if (state_node_[key]) {
        ...
        field.entity_present = comp["entity_present"] ? comp["entity_present"].as<bool>() : false;
        if (comp["entity_name"]) field.entity_name = comp["entity_name"].as<std::string>();
        if (comp["entity_pose"]) {
          field.translation = comp["entity_pose"]["translation"] ? comp["entity_pose"]["translation"].as<double>() : 0.0;
          field.roll = comp["entity_pose"]["roll"] ? comp["entity_pose"]["roll"].as<double>() : 0.0;
          field.pitch = comp["entity_pose"]["pitch"] ? comp["entity_pose"]["pitch"].as<double>() : 0.0;
          field.yaw = comp["entity_pose"]["yaw"] ? comp["entity_pose"]["yaw"].as<double>() : 0.0;
        }
      }
    };
  ...
  rclcpp::Service<aic_flowstate_interface::srv::GetTaskboardState>::SharedPtr get_state_service_;
};
