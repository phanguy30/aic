...
SAVE_PATH = os.path.join(HOME_DIR, "aic_dataset", "aic_dataset_holes")
TARGET_IMAGE_COUNT = 1000

class GenerateDataInsertionHoles(Policy):
    ...
    def insert_cable(
        ...
        self.get_logger().info("Starting YOLO Data Generation Policy for Insertion Holes!")
        self._task = task
        ...
        possible_ports = [f"port_{i}_link_entrance" for i in range(5)] + \
                         [f"sfp_port_{i}_link_entrance" for i in range(5)] + \
                         [f"sc_port_{i}_link_entrance" for i in range(5)] + \
        ...
        while self._image_count < TARGET_IMAGE_COUNT:
            ...
            if not yolo_lines:
                self.get_logger().warn("No entrance holes visible in image, skipping...")
                continue
            ...
            self.get_logger().info(f"Saved generated image {self._image_count:05d} with {len(yolo_lines)} labeled insertion holes.")
            self._image_count += 1
